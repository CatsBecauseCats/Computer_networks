...
void parse_file_length(char* str, size_t *size) {
	*size = 0;
	while (*str >= '0' && *str <= '9') {
		*size = *size * 10 + (*str - '0');
		str++;
...
int main() {
	...
	char* bytes;
	size_t maxsize = MB_SIZE * 2;
	size_t rsize;

	printf("Server started!\n");
	// printf("Input max file size: ");
	// fgets(buf, MSG_LEN, stdin);
	// parse_file_max_length(buf,&maxsize);
	...
	for(;;) {
		if (recvfrom(sock, fileName, MSG_LEN, 0, &client_addr, &slen) == -1) {
		...
		if (rsize <= maxsize) {
			...
			for (int k = 0; k < packets; k++) {
				...
				if (recvfrom(sock, bytes, MSG_LEN, 0, &client_addr, &slen) == -1) {
